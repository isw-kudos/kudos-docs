...
The migration interface is accessible at `https://[CONNECTIONS_URL]/boards/app/admin/migration` to select which Activities to migrate (ie ignore completed/deleted). For some explanation of the interface, see [Activity Migration User Interface](/boards/cp/migration-interface).
